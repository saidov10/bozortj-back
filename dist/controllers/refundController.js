...
exports.resolveRefundDispute = exports.processRefundRequest = exports.createRefundRequest = void 0;
...
const createRefundRequest = async (req, res) => {
    try {
        ...
        if (req.user.role !== 'BUYER')
            return res.status(403).json({ message: 'Only buyers can request refunds' });
        ...
        const existingRequest = await prisma_1.default.refundRequest.findUnique({
            where: { orderId }
        });
        if (existingRequest) {
        ...
        const refundRequest = await prisma_1.default.$transaction(async (tx) => {
        ...
        for (const shopId of uniqueShopIds) {
            ...
            if (shop) {
                await (0, notificationService_1.createNotification)(shop.userId, 'Return & Refund Requested', `A return has been requested for Order #${order.id.substring(0, 8)}. Reason: ${reason}`);
            }
        }
...
const processRefundRequest = async (req, res) => {
    try {
        ...
        const refundRequest = await prisma_1.default.refundRequest.findUnique({
            where: { orderId },
            include: {
                order: {
                    include: { items: true }
        ...
        // Notify buyer
        await (0, notificationService_1.createNotification)(refundRequest.order.userId, `Refund Request ${status}`, `Your refund request for Order #${orderId.substring(0, 8)} has been updated to ${status}. Note: ${explanation || 'None'}`);
...
const resolveRefundDispute = async (req, res) => {
    try {
        ...
        const resolvedRequest = await prisma_1.default.$transaction(async (tx) => {
            const request = await tx.refundRequest.update({
                where: { orderId },
                data: {
        ...
        // Notify buyer
        await (0, notificationService_1.createNotification)(refundRequest.order.userId, 'Refund Dispute Resolved', `Admin resolved the dispute for Order #${orderId.substring(0, 8)} as ${status}.`);
        return res.status(200).json({
...
exports.resolveRefundDispute = resolveRefundDispute;
